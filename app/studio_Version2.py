...
def process(
    ...
    # MULTI-GPU: Acquire a free GPU before proceeding
    gpu_idx = acquire_free_gpu()
    ...
    job_params = {
        ...
        'gpu_idx': gpu_idx,   # MULTI-GPU: record which GPU this job uses
        'device': device
    }
...
def end_process():
    ...
    with job_queue.lock:
        if job_queue.current_job:
            ...
            # MULTI-GPU: Release GPU when cancelling
            if hasattr(job_queue.current_job, "gpu_idx"):
                release_gpu(job_queue.current_job.gpu_idx)
...
def monitor_job(job_id=None):
    ...
    while True:
        ...
        if job.status == JobStatus.PENDING:
            ...
            # MULTI-GPU: Release GPU on completion
            if hasattr(job, "gpu_idx"):
                release_gpu(job.gpu_idx)
            ...
            # MULTI-GPU: Release GPU on cancel
            if hasattr(job, "gpu_idx"):
                release_gpu(job.gpu_idx)
